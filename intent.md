...
**Business Question**: What is net revenue after region-specific tax calculations (VAT vs. sales tax)?

**The Polars Solution**: Mapping dictionaries or join with tax config dataframe.

```python
# Join with regional tax rates
regional_financials = orders_df.join(
    tax_config_df,
    on="region"
).with_columns(
...
**Output Table**: `regional_financials` with pre-calculated `tax_amount` and `net_revenue`.
...
## The Three Gold Marts

### 1. Finance Mart (`fct_finance_revenue`)
**Consumers**: Finance team, CFO dashboards

**Grain**: Daily revenue by region, product category, and tax jurisdiction

**Key Metrics**:
- `gross_revenue`: Sum of order totals
- `tax_amount`: Region-specific tax calculations (from Enriched Silver)
- `net_revenue`: Post-tax revenue
- `locked_capital`: Inventory carrying costs (from Inventory Risk table)
- `cash_recovery_potential`: $3.8M working capital opportunity from rationalization

**Source Tables**: `regional_financials`, `inventory_risk`
...
**Grain**: Customer acquisition cohorts with channel attribution and recovery metrics

**Key Metrics**:
- `acquisition_channel`: (Phone, Social Media, Organic, etc.)
- `predicted_clv_score`: Channel-weighted lifetime value estimate
- `cart_recovery_rate`: % of abandoned carts that converted (from Attribution table)
- `cohort_retention_m3`: Month-3 retention rate by signup cohort
- `at_risk_customers`: Daily count of customers in "danger zone" (from Retention Signals)

**Source Tables**: `attributed_purchases`, `customer_retention_signals`
...
- `stock_level`: Current inventory units
- `restock_priority_flag`: Low stock + Healthy velocity = priority restock
- `rationalization_flag`: High risk + Low utilization = delisting candidate

**Source Tables**: `sales_velocity`, `inventory_risk`
