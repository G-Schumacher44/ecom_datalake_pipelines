...
def test_semantic_checks_cart_attribution() -> None:
    ...
    config.semantic_checks = {
        "int_cart_attribution": [
            {"name": "converted_requires_order_id", "expr": "cart_status = 'converted' and order_id is null"},
            {"name": "abandoned_requires_null_order_id", "expr": "cart_status = 'abandoned' and order_id is not null"},
            {"name": "abandoned_value_positive_for_abandoned", "expr": "cart_status = 'abandoned' and abandoned_value <= 0"},
            {"name": "time_to_purchase_hours_non_negative", "expr": "time_to_purchase_hours < 0"},
        ]
    }
...
def test_semantic_checks_product_performance() -> None:
    ...
    config.semantic_checks = {
        "int_product_performance": [
            {"name": "units_returned_le_units_sold", "expr": "units_returned > units_sold"},
            {"name": "return_rate_le_one", "expr": "return_rate > 1.0 + {ratio_epsilon}"},
            {"name": "cart_to_order_rate_le_one", "expr": "cart_to_order_rate > 1.0 + {ratio_epsilon}"},
            {"name": "net_margin_le_gross_margin", "expr": "net_margin > gross_margin + {ratio_epsilon}"},
        ]
    }
...
def test_semantic_checks_shipping_economics() -> None:
    ...
    config.semantic_checks = {
        "int_shipping_economics": [
            {"name": "shipping_margin_matches_components", "expr": "abs(shipping_margin - (shipping_cost - actual_shipping_cost)) > {ratio_epsilon}"},
            {"name": "shipping_margin_pct_null_when_zero_cost", "expr": "shipping_cost = 0 and shipping_margin_pct is not null"},
        ]
    }
