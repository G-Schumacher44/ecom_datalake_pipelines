...
SLOGAN_REGEX = re.compile(r"""<p align="center">\s*<sub>✨ Transform the data. Tell the story. Build the future. ✨</sub>\s*</p>""", re.DOTALL | re.IGNORECASE)

# Regex to match the Nav block
NAV_REGEX = re.compile(r"""<p align="center">\s*<a href="[^"]+">.*?🏠.*?</a>.*?</p>""", re.DOTALL | re.IGNORECASE)

# Regex to match the Timestamp block
TIME_REGEX = re.compile(r"""<p align="center">\s*<sub>Last updated:.*?</sub><br>""", re.DOTALL | re.IGNORECASE)

# Combined footer regex for cleaner replacement if they are adjacent
# We will rely on finding Slogan and looking backwards, as implemented in logic below
...
def update_file(file_path):
    ...
    if slogan_match:
        # We found the slogan.
        # Now we search backwards to see if there is an existing timestamp and/or nav block immediately preceding it.
        # We define "immediately preceding" as "only whitespace in between".
        
        start_cut = slogan_match.start()
        end_cut = slogan_match.end()
        
        # 1. Look for Timestamp immediately before start_cut
        # We scan the text before start_cut
        pre_text = content[:start_cut]
        
        # Check if pre_text ends with Timestamp block (ignoring whitespace)
        # We'll use a loop to consume preceding blocks if they match
        ...
        if not clean_pre:
            ...
            # Let's standardize: Ensure there is a '---' before the footer if the file is long enough.
...
def main():
    ignore_dirs = {".venv", ".git", "node_modules", "dbt_packages", "__pycache__", ".mypy_cache", ".ruff_cache", "coverage_html", ".claude", ".gcloud", ".github", ".vscode", "target", "logs", "plugins"}
