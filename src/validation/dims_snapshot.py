...
def validate_snapshot(table: str, path: str, partition_key: str, run_date: str) -> dict:
    ...
    if df is None:
        return {"status": "FAIL", "rows": 0, "message": "Partition not found or unreadable"}
...
def main() -> int:
    ...
    for r in results:
        ...
        report_lines.append(f"| {r['table']} | {status_emoji} {r['status']} | {r['rows']:,} | {r['message']} | {path_display} |")
    ...
    logger.info(f"Dims snapshot validation {overall_status}", tables=[r["table"] for r in results])
    
    return handle_exit(overall_status, args.enforce_quality, os.getenv("PIPELINE_ENV", "dev"))
