{{ config(
    materialized='table',
    post_hook=[
        "COPY (SELECT * FROM {{ this }}) TO '{{ var('silver_base_path') }}/quarantine/product_catalog' (FORMAT PARQUET, PARTITION_BY (category), OVERWRITE_OR_IGNORE)"
    ]
) }}

{#
QUARANTINE PATTERN: product_catalog
Captures all INVALID product records with reasons
#}
...
-- Quarantine: Return only INVALID products
select
    product_id,
    product_name,
    category,
