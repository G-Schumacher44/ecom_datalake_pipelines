{{ config(
    materialized='table',
    post_hook=[
        "COPY (SELECT * FROM {{ this }}) TO '{{ var('silver_base_path') }}/return_items' (FORMAT PARQUET, PARTITION_BY (return_dt), OVERWRITE_OR_IGNORE)"
    ]
) }}
...
{#
TRANSACTION TABLE PATTERN: return_items (MOST COMPLEX)
- 3 FKs: return_id -> returns, order_id -> orders, product_id -> product_catalog
- Validate: quantity_returned > 0, amounts >= 0
...
-- Reference dimensions for FK validation (3 FKs!)
dim_returns as (
    select distinct
...
validated as (
    select
        ...
        -- Deduplication: keep most recent record per return_id/product_id
        row_number() over (
