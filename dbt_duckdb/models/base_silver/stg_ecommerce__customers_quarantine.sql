{{ config(
    materialized='table',
    post_hook=[
        "COPY (SELECT * FROM {{ this }}) TO '{{ var('silver_base_path') }}/quarantine/customers' (FORMAT PARQUET, PARTITION_BY (signup_dt), OVERWRITE_OR_IGNORE)"
    ]
) }}

{#
QUARANTINE PATTERN: customers
Captures all INVALID customer records with reasons
#}
...
-- Quarantine: Return only INVALID customers
select
    customer_id,
    email,
    signup_date,
