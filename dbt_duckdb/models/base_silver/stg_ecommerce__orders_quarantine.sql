{{ config(
    materialized='table',
    post_hook=[
        "COPY (SELECT * FROM {{ this }}) TO '{{ var('silver_base_path') }}/quarantine/orders' (FORMAT PARQUET, PARTITION_BY (order_dt), OVERWRITE_OR_IGNORE)"
    ]
) }}
...
{#
QUARANTINE PATTERN: orders
Captures all INVALID rows with reasons for debugging and data quality monitoring
#}
