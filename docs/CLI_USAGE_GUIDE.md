...
**Default output**: `docs/planning/BRONZE_PROFILE_REPORT.md`

```bash
# Generate quality report with schema drift detection
python scripts/describe_parquet_samples.py \
  --date-range 2020-01-01..2020-12-31 \
  --output docs/planning/BRONZE_PROFILE_REPORT.md
```
...
| `--output` | string | `docs/planning/BRONZE_PROFILE_REPORT.md` | Markdown report output path |
...
./scripts/report_bronze_sizes.sh \
  acme-analytics-raw \
  ecom/raw \
  docs/planning/BRONZE_SIZES_2026_Q1.md
```
...
```yaml
pipeline:
  bronze_bucket: "acme-analytics-raw"
...
cat docs/planning/BRONZE_PROFILE_REPORT.md
```
...
python scripts/describe_parquet_samples.py \
  --months 2026-01 \
  --output docs/planning/BRONZE_PROFILE_2026_01.md

# Compare schemas
diff docs/planning/BRONZE_SCHEMA_MAP.json docs/planning/BRONZE_SCHEMA_MAP_2026_01.json
```
...
grep "⚠️" docs/planning/BRONZE_PROFILE_REPORT.md
```
...
./scripts/report_bronze_sizes.sh \
  acme-analytics-raw \
  ecom/raw \
  docs/planning/BRONZE_SIZES_CURRENT.md

# Review per-table costs
cat docs/planning/BRONZE_SIZES_CURRENT.md
```
...
# Compare to baseline
with open("docs/planning/BRONZE_SCHEMA_MAP.json") as f:
...
- [Bronze Profile Report](planning/BRONZE_PROFILE_REPORT.md) - Latest quality analysis
- [Data Contract](planning/DATA_CONTRACT.md) - Bronze → Silver type mappings
- [Data Dictionary](planning/DATA_DICTIONARY.md) - Field-level documentation
