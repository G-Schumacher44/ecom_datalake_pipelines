...
1. **Expand Pydantic Schemas** (30 min)
   - Add 5 missing table schemas
   - Update validation script

2. **Create Silver Quality Validator** (1 hour)
   - `src/validation/silver_quality.py`
   - Compare Bronze vs Silver row counts
   - Calculate pass rates
   - Write to `data/metrics/silver_quality/`

3. **Wire Into DAG** (30 min)
   - Add Phase 1.5: Silver quality validation
   - Update Phase 0 to use observability framework
   - Add run_id generation

4. **Add Bronze Metadata Validator** (1 hour)
   - `src/validation/bronze_metadata.py`
   - Fast schema/row count/partition checks
   - Write to `data/metrics/data_quality/`
